...
namecheap.prototype = {
	get domains() {
		...
		return {
			...
			setRegistrarLock: function (domain, lock, callback) {
				...
				return instance.command('domains.setRegistrarLock', {
					DomainName: domain,
			...
			dns: {
				...
				setCustom: function (domain, nameservers, callback) {
					...
					return instance.command('domains.dns.setCustom', {
						SLD: domain.shift(),
						TLD: domain.join('.'),
			...
			ns: {
				create: function (domain, nameserver, ip, callback) {
					...
					domain = domain.split('.');
					return instance.command('domains.ns.create', {
						SLD: domain.shift(),
						TLD: domain.join('.'),
				...
				delete: function (domain, nameserver, callback) {
					...
					if (!nameserver)
						throw new Error('You must include a nameserver.');
					domain = domain.split('.');
					return instance.command('domains.ns.delete', {
						SLD: domain.shift(),
						TLD: domain.join('.'),
						Nameserver: nameserver
					}, callback);
				...
				getInfo: function (domain, nameserver, callback) {
					...
					if (!nameserver)
						throw new Error('You must include a nameserver.');
					domain = domain.split('.');
					return instance.command('domains.ns.getInfo', {
						SLD: domain.shift(),
						TLD: domain.join('.'),
						Nameserver: nameserver
					}, callback);
				...
				update: function (domain, nameserver, old, ip, callback) {
					...
					domain = domain.split('.');
					return instance.command('domains.ns.update', {
						SLD: domain.shift(),
						TLD: domain.join('.'),
						Nameserver: nameserver,
						OldIP: old,
	...
	get ssl() {
		...
		return {
			...
			create: function (type, years, sta, code, callback) {
				if (!type)
	...
	get users() {
		...
		return {
			...
			createAddFundsRequest: function (amount, type, url, callback) {
				...
				return instance.command('users.createAddFundsRequest', {
					Amount: amount,
					PaymentType: type,
					ReturnUrl: url
				}, callback);
	...
	command: async function (cmd, params, callback, method) {
